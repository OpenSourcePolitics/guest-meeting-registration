...
module Decidim::Meetings
  describe RegistrationSerializer do
    describe "#serialize" do
      ...
      let!(:guest_registration) { create(:guest_meeting_registration, create_registration: false, organization: registration.meeting.organization, meeting: registration.meeting) }
      ...
      context "when there are not a questionnaire" do
        ...
        context "when there is no guest" do
          it "includes the guest information" do
            expect(subject.serialize).to include(guest: "No")
          end
        end

        context "when there is a guest" do
          let!(:guest_registration) { create(:guest_meeting_registration, create_registration: false, user: registration.user, organization: registration.meeting.organization, meeting: registration.meeting) }

          it "includes the guest information" do
            expect(subject.serialize).to include(guest: "Yes")
          end
        end
      ...
      context "when questionnaire enabled" do
        ...
        context "when there is a guest" do
          let!(:guest_registration) { create(:guest_meeting_registration, create_registration: false, user:, organization: registration.meeting.organization, meeting: registration.meeting) }
